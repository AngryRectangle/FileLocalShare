...
public class SocketWrapper {
    ...
    public class FileReceiver {
        ...
        private void writeFile(File file) throws IOException, InterruptedException {
            ...
            byte[] buffer = new byte[com.radioactiv_gear_project.core.NetworkInteraction.IO_BUFFER_SIZE];
            int readed;
            int needToRead;
            while (remaining > 0) {
                if (remaining < com.radioactiv_gear_project.core.NetworkInteraction.IO_BUFFER_SIZE)
                    needToRead = Math.min((int) remaining, buffer.length);
                else
                    needToRead = com.radioactiv_gear_project.core.NetworkInteraction.IO_BUFFER_SIZE;
                readed = reader.read(buffer, 0, needToRead);
                fileStream.write(buffer, 0, readed);
                fileStream.flush();
                remaining -= readed;
                totalReceived += readed;
                sleep(com.radioactiv_gear_project.core.NetworkInteraction.INFORMATION_RECEIVING_SLEEP_TIME);
            }
            fileStream.close();
        }
    }
